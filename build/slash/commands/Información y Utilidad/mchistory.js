...
export default class mchistory extends Command {
    ...
    async run(interaction) {
        ...
        const res = await fetch(`https://mc-heads.net/minecraft/profile/${interaction.options.getString('account', true)}`)
        ...
        return await interaction.reply({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setFields(res.name_history?.map((i) => ({
...
function addCeroDay(numero) {
    if (numero < 10)
        return '0' + numero;
    else
        return numero + '';
}
function parserTimeStamp(date) {
    date = new Date(date);
    return (addCeroDay(date.getDate()) +
        '-' +
...
//# sourceMappingURL=mchistory.js.map