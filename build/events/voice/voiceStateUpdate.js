...
export default class VoiceStateUpdate extends BaseEvent {
    async run(client, oldState, newState) {
        ...
        if (newUserVoiceChannel.id !== vc.id)
            return;
        ...
        if (!player || player.waitingMessage)
            return;
        if (player.stayInVc) {
            player.pause(true);
            if (client.settings.debug === 'true')
                logger.debug('AutoPaused (24/7) | ' + player.guild.name + ' | ' + player.queue.current?.requester.displayName);
        }
        else {
            const embed = new EmbedBuilder()
                .setDescription(translate(keys.voice_update.leaving, {
        ...
        if (!voiceMembers || voiceMembers < 1) {
            const newPlayer = await client.music.createNewPlayer(oldState.guild.members.cache.get(client.user.id)?.voice.channel, player.textChannelId ?? '', 100);
            ...
            if (player.waitingMessage)
                player.waitingMessage.edit({
                    embeds: [
                        new EmbedBuilder()
                            .setDescription(translate(keys.voice_update.alone, {
                            channel: oldUserVoiceChannel.toString(),
