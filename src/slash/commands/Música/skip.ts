...
export default class skip extends Command {
    ...
    async run(client: Client, interaction: interactionCommandExtended) {
        ...
        if (!interaction.member.voice) return message.sendMessage({ embeds: [errorEmbed] }, false).catch((e: any) => { logger.debug(e) })
        ...
        if (interaction.member?.voice.channelId != vc?.id) {
            ...
            return message.sendMessage({ embeds: [errorembed] }, false).catch((e: any) => { logger.debug(e) })
        }
        ...
        const embed = new EmbedBuilder()
            ...
            .setFooter({ text: interaction.user.username, iconURL: interaction.user.displayAvatarURL() })
        message.sendMessage({ embeds: [embed] }, false).catch((e: any) => { logger.debug(e) })
