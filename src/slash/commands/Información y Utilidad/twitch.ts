...
export default class Twitch extends Command {
            ...
            options: [
                {
                    ...
                    name: 'add',
                    description:
                        'Add a notification for when a streamer goes live',
                    options: [
                        {
                ...
                },
                {
                    ...
                    name: 'remove',
                    description:
                        'Remove a notification for when a streamer goes live',
                    options: [
                        {
                            type: ApplicationCommandOptionType.String,
                            name: 'streamer',
    ...
    async add(interaction: ChatInputCommandInteraction) {
        ...
        const streamerData = (await streamerReq.json()) as {
            data: TwitchUser[]
        }
        ...
        const subreq = await fetch(
            `https://api.twitch.tv/helix/eventsub/subscriptions?id=${2}`,
            {
                method: 'POST',
                headers: {
                    ...headers,
        ...
        return await interaction.reply({
            content:
                translate(keys.twitch.now_following, {
