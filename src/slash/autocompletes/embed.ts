...
export default class Embed extends Autocomplete {
    ...
    override async run(interaction: AutocompleteInteraction) {
        ...
        if (!focused) {
            ...
            if (!Color.isHex(focused))
                suggestions.push({ name: invalidInput, value: '#000000' })
            for (
                let i = 0, random = Math.floor(Math.random() * values.length);
                i < 24;
                i++, random = Math.floor(Math.random() * values.length)
            )
                suggestions.push({
                    name: `#${values[random].toString(16)}`,
                    value: `${values[random]}`,
                })
        } else if (focused.startsWith('0x')) {
            if (!Color.isHex0x(focused))
                suggestions.push({ name: invalidInput, value: '0x000000' })
            for (
                let i = 0, random = Math.floor(Math.random() * values.length);
                i < 24;
                i++, random = Math.floor(Math.random() * values.length)
                ...
                suggestions.push({
                    name: `0x${values[random].toString(16)}`,
                    value: `${values[random]}`,
                })
        } else if (focused.startsWith('rgb')) {
            if (!Color.isRGB(focused))
                suggestions.push({ name: invalidInput, value: 'rgb(0, 0, 0)' })
            for (let i = 0; i < 24; i++) {
            ...
            suggestions.push(
                ...colors
                    .filter(([key]) =>
                        suggestions.some(({ name }) => name !== key),
                    )
                    .map(([name, value]) => ({ name, value: `${value}` })),
            )
        }
