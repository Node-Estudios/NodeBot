...
export default class Color {
    ...
    constructor (color: string) {
        ...
        if (Color.isHex0x(color)) {
            if (color.length === 5) color = `0x${color[1]}${color[1]}${color[2]}${color[2]}${color[3]}${color[3]}`
        ...
        if (Color.isName(color)) {
            const match = Colors[color as keyof typeof Colors].toString(16).match(/[0-9A-Za-z]{2}/g) ?? []
            const [r, g, b] = match
    ...
    get rgbString (): `rgb(${number}, ${number}, ${number})` {
        return `rgb(${this.#rgb.join(', ')})` as `rgb(${number}, ${number}, ${number})`
    }
    ...
    static isHex0x (color: string): boolean {
    ...
    static isName (color: string): boolean {
        return Object.keys(Colors).some(c => c === color)
    }

    static isNumber (color: string): boolean {
